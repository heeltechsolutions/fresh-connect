...
const Accommodation = require("../models/Accommodation.js"); // Import the accommodation model
...
router.get("/inactive_accommodations", async (req, res) => {
    const { page = 1, search = "" } = req.query;
    const limit = 10; // Number of items per page
    const skip = (page - 1) * limit;
  
    try {
      // Count accommodations that match the criteria
      const totalAccommodations = await Accommodation.countDocuments({
        placeName: { $regex: search, $options: "i" }, // Case-insensitive search by place name
        status: "inactive", // Filter by inactive status
        Useremail: { $exists: true, $ne: "" }, // Ensure Useremail exists and is not empty
      });
      ...
      // Respond with the filtered accommodations
      res.json({
        accommodations,
        totalPages: Math.ceil(totalAccommodations / limit),
        currentPage: parseInt(page),
